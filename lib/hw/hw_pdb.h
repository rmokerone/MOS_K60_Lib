...
//使能PDBx中断
#define LPLD_PDB_EnableIrq()    enable_irq(INT_PDB0 - 16)
//禁用PDBx中断
#define LPLD_PDB_DisableIrq()   disable_irq(INT_PDB0 - 16)
//软件触发PDBx计数器开始工作
#define LPLD_PDB_SoftwareTrigger()      (PDB0_SC|=PDB_SC_SWTRIG_MASK)
...
//PDBx触发ADCx模块配置函数
uint8 LPLD_PDB_AdcTriggerCfg(ADC_MemMapPtr , uint32, uint16);
//PDBx触发DACx模块配置函数
uint8 LPLD_PDB_DacTriggerCfg(DAC_MemMapPtr , uint32, uint16);
