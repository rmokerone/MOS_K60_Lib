/**********************************************************************
	...
	> Created Time: Thu 20 Mar 2014 01:12:57 AM CST
 *********************************************************************/
...
//使能ADCx相应转换通道的AD采集功能
uint8 LPLD_ADC_Chn_Enable(ADC_MemMapPtr, AdcChnEnum_Type);
//软件触发模式获取AD转换结果
uint16 LPLD_ADC_Get(ADC_MemMapPtr, AdcChnEnum_Type);
//使能ADCx中断
uint8 LPLD_ADC_EnableIrq(ADC_InitTypeDef);
//禁用ADCx中断
uint8 LPLD_ADC_DisableIrq(ADC_InitTypeDef);
//使能ADCx转换通道，用于软件中断、硬件触发等模式
void LPLD_ADC_EnableConversion(ADC_MemMapPtr , AdcChnEnum_Type, uint8, boolean);
//获取AD转换结果
uint16 LPLD_ADC_GetResult(ADC_MemMapPtr, uint8);
//获取SC1寄存器中COCO位置1的编号
uint8 LPLD_ADC_GetSC1nCOCO(ADC_MemMapPtr);
