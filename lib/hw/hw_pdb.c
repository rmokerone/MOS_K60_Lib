...
uint8 LPLD_PDB_AdcTriggerCfg(ADC_MemMapPtr adcx, uint32 cfg, uint16 delay)
{
  ...
  PDB0_C1(n) |= cfg;        //根据参数配置BB模式、选择预触发输出、使能预触发
  PDB0_DLY(n, m) = (bus_clk * delay)/(prescaler * mult);    //配置ADCn的m组预触发延时值
  
  PDB0_SC |= PDB_SC_LDOK_MASK; //加载值
...
uint8 LPLD_PDB_DacTriggerCfg(DAC_MemMapPtr dacx, uint32 cfg, uint16 int_delay)
{
  ...
  PDB0_INTC(n) |= cfg;     //配置使能外部触发或者内部间隔触发
  PDB0_INT(n) = (bus_clk * int_delay)/(prescaler * mult);    //配置DACn的间隔触发时间
